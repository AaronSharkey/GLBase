...
private:
    // model data
    vector<Tex> textures_loaded; // Global textures store so we aren't duplicating during texture load
    vector<Mesh> meshes;
    string directory;
...
inline Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene) {
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
        Vertex vertex;
        // Create a temp vector for transfer purposes
        glm::vec3 vector;
    ...
    if (mesh->mMaterialIndex >= 0) {
        ...
        vector<Tex> diffuseMaps = loadMaterialTextures(material,
                                                       aiTextureType_DIFFUSE, "texture_diffuse");
        textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());

        vector<Tex> specularMaps = loadMaterialTextures(material,
...
inline vector<Tex> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName) {
    vector<Tex> textures;
    unsigned int texture_count = mat->GetTextureCount(type);
    std::cout << "Texture count (" << typeName << "): " << mat->GetTextureCount(type) << std::endl;
